...
const ColorPaletteModal = ({ navigation }) => {
    /* Initializing 'color name' State */
    const [name, setName] = useState('')

    /* Initializing 'Selected colors' state */
    ...
    /* Handling the color switch button */
    const handleValueChange = useCallback((value, color) => {

        /* If the color button is set to ON... */
        if (value === true) {
            // Add color to the bottom of the list
            setSelectedColors(colors => [...colors, color])
        }

        /* Else, if the color button is set to OFF... */
        else {
            // Filter on the list the selected color
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={styles.colorNameRow}>
                        ...
                        <Switch
                            /* Convert Object to array (Android), evaluate if it's on Home screen and convert to boolean */
                            value={
